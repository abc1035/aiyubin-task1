...
class SingleStageDetector(BaseDetector):
    ...
    def forward_train(self,
        ...
        if len(temp) == 2:
            ...
            losses = {**losses2, **losses3}
            return losses
        elif len(temp) == 4:
            x, x_rgb, x_depth, depth = temp
            """for item in x:
                print(item.shape)"""
            losses1 = self.bbox_head.forward_train(x, img_metas, gt_bboxes,
                                                   gt_labels, gt_bboxes_ignore, depth=depth)
            losses1 = self.updatedic(losses1, "_main_branch")
            losses2 = self.bbox_head.forward_train(x_rgb, img_metas, gt_bboxes,
                                                   gt_labels, gt_bboxes_ignore, depth=None)
            losses2 = self.updatedic(losses2, "_rgb_branch")
            losses3 = self.bbox_head.forward_train(x_depth, img_metas, gt_bboxes,
    ...
    def simple_test(self, img, img_metas, rescale=False):
        ...
        """
        # feat = self.extract_feat(img, 1, img_metas, branch="rgb")
        # feat = self.extract_feat1(img)
    ...
    def aug_test(self, imgs, img_metas, rescale=False):
        ...
        feats = self.extract_feat_fuse_after(img, 1, img_metas, branch="rgb")
