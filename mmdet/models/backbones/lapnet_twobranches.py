...
@File ：lapnet_element_wise_times.py
@IDE ：PyCharm
...
class Lapnet_twoBranches(nn.Module):
    ...
    def forward(self, x, *args, **kwargs):
        ...
        """if len(args) == 0:
            ...
            #    depth = depth.cuda.set_device(x.device)
            # print(depth.shape)
        ...
        depth = self.depth_extractor(x)
        # print(depth.mean())
        """if kwargs.get("iteration", -1) > 10000:
            x = torch.cat([x, depth], 1)
        else:
            x = torch.cat([x, depth.detach()], 1)"""
        # x = x.detach()
        # print(x.shape)
        ...
        for item in x_depth:
            print(item.shape)
        print("shit")"""
        """for i, item in enumerate(x_rgb):
            item = np.array((item.sum(dim=1) / item.shape[1]).squeeze().cpu()).tolist()
            f = open("/home/ayb/work_dirs/x{}.txt".format(i), "w")
            f.write(str(item))
            f.close()
        assert 1==2"""
        if self.plot_img and False:
            self.plot(depth)
        # return self.fuse(x_depth, x_rgb), x_rgb, x_depth, depth
        return x_rgb, x_depth, depth
        # return self.fuse(x_depth, x_rgb), depth
        # return x, depth
        # return x
