...
img_dir = '/home/ayb/work_dirs/rgbinfer/'
...
def load_annotations(ann_path="/home/ayb/UVM_Datasets/voc_valid3.json"):
    f = open(ann_path)
    dic = f.readline()
    dic = eval(dic)
    f.close()
...
def check(bbox_list, pre_bbox, pre_label):
    ...
    while True:
        ...
        for index in gt_list:
            ...
            # print(overlap.sum())
            if labels[index] == pre_label[pre_list[0]] and overlap.sum() > 0.3:
...
def find_mistakes(dic, images, falseimage):
    # f = open("/home/ayb/work_dirs/rgbinfer/wrong.txt", "w")
    # cnt = 0
    ...
    while not images.empty():
        ...
        """print(bbox_list)"""
        pre_bbox, pre_label = pre_dic[file_name]
        if not check(bbox_list, pre_bbox, pre_label):
...
if __name__ == "__main__":
    ...
    images = dic['images']
    inference_all(images)
    qimage = mp.Queue()
    falseimage = mp.Queue()
    for image in tqdm(images):
        qimage.put(image)
    p1 = Process(target=find_mistakes, args=(dic, qimage, falseimage))
    p2 = Process(target=find_mistakes, args=(dic, qimage, falseimage))
    """p3 = Process(target=find_mistakes, args=(dic, qimage, falseimage))
    p4 = Process(target=find_mistakes, args=(dic, qimage, falseimage))
    p5 = Process(target=find_mistakes, args=(dic, qimage, falseimage))
    p6 = Process(target=find_mistakes, args=(dic, qimage, falseimage))
    p7 = Process(target=find_mistakes, args=(dic, qimage, falseimage))"""
    p1.start()
    p2.start()
    # p3.start()
    # p4.start()
    ...
    while True:
        if qimage.empty():
            f = open("/home/ayb/work_dirs/rgbinfer/wrong.txt", "w")
            while not falseimage.empty():
                file_name = falseimage.get()
                f.write(file_name + "\n")
            f.close()
            break
